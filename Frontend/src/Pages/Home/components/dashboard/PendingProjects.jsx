...
import UserProjects from '../../User/userSidebar/UserProjects'
...
const PendingProjects = () => {
    ...
    return (
        <div className={`${isDarkMode ? "dark" : ""}`}>
            <UserProjects />
            <div className='flex-1 px-2 pl-[26%]'>
                <div className=' p-5 pt-2 '>
                    {Data.map((data, index) => (
                        <div className={`${'w-full flex items-center justify-between bg-[#f6f6f6] rounded-xl mb-4 px-4 py-4 dark:bg-[#414141]'}`} key={index}>
                            ...
                            <div>
                                <h1 className='font-medium text-lg dark:text-whitebg'>{data.title}</h1>
                                <p className='font-normal text-[15px] leading-5 dark:text-whitebg'>{data.content}</p>
                            </div>
                        </div>
                    ))}
                </div>
